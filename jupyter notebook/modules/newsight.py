...
class Pickle2DF:
    ...
    def get_dataframe_from_csv(delf, df):
        df['category'] = df['category'].apply(lambda x : x if x == '' else x.split(' | '))
        df['date_tmp'] = df['date'].apply(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d').toordinal())
        return df
...
class GetKeyword:
        ...
        def remove_na_category(self,df):
            idx_na_cat = df[df['category'].apply(len)==0].index
